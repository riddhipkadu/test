...
public class SendMailServiceImpl implements SendMailService {
	...
	public void sendMail(String email_subject, String email_body, InternetAddress[] addresses, InternetAddress[] addressCC ,String id) {
		//System.out.println(addressCC.toString());
		...
		try {
			...
			if (res != null) {
				...
				while (iterator.hasNext()) {
					...
					String exec_contract_division = objects[3].toString();
					String exec_contract_date = "";
					String exec_contract_execution_date = "";
					String exec_contract_end_date = "";
					
					if(objects[5]!=null)
						exec_contract_date = sdfOut.format(sdfIn.parse(objects[5].toString()));
					if(objects[6]!=null)
						exec_contract_execution_date = sdfOut.format(sdfIn.parse(objects[6].toString()));
					
					String user_requested_by_fullname = userService.getUserFullNameById(Integer.parseInt(objects[13].toString()));
					String user_responsible_fullname = objects[8].toString()+" "+objects[9].toString();
					assign_person_mail = objects[10].toString();

					
					if(objects[7]!=null)
						exec_contract_end_date = sdfOut.format(sdfIn.parse(objects[7].toString()));
					//String assigned_by = session.getAttribute("fullName").toString();
					email_body += "<tr>" + "<td><a href='" + project_url + "updateTab?cont_id=" + contract_id + "'>"+ contract_id + "</a></td>" 
							+ "<td>" + orga_name + "</td>" 
							+ "<td>" + loca_name + "</td>"
							+ "<td>" + dept_name + "</td>" 
							+ "<td>" + exec_contract_division + "</td>" 
							+ "<td>" + exec_contract_date + "</td>" 
							+ "<td>" + exec_contract_execution_date + "</td>" 
							+ "<td>" + exec_contract_end_date + "</td>" 
				...
				try {
					for (int i = 0; i < email.size(); i++) {
						addressCC[i] = new InternetAddress(email.get(i));
						//System.out.println(addressCC[i] );
					}
				} catch (AddressException e) {
					e.printStackTrace();
				}
	...
	// Method created by : Tejashri Zurunge
		...
		// pre-execution contract
		@Override
		public void sendExecutionAssignMailToResponsiblePerson(int exec_contra_id, HttpSession session) {
			try {
				List<Object> res = sendMailDao.getExecutionDetailsByContractId(exec_contra_id);
				if (res != null) {
					String email_body = "";
					String assign_person_mail = "";
					String cont_agreement_name = "";
					int orga_id = 0;
					email_body += "<div style='margin:0 auto;width:100%;height:auto;padding:20px;'>";
					email_body += "<h2 style='font-size:18px;'>Dear User,</h2>";
					email_body += "<p style='text-align:justify;width:70%;'>The following Executed contract is assigned to you. You are requested to follow the link by clicking on the 'Contract ID'.</p>"
					...
					email_body += "<table style='width:80%;' border='1'>" + "<thead>"
							...
							+ "<th>Execution Date</th>" 
							 + "<th>Term Period of the Agreement (In months)</th>" 
							//+ "<th>Assigned by</th>"
							+ "<th>Assigned to</th>" 
							+ "<th>Expiry date</th>" 
							+ "</tr>"
					...
					while (iterator.hasNext()) {
						...
						String dept_name = objects[2].toString();
						if(objects[3]!=null)
						cont_agreement_name = objects[3].toString();
						String exec_contract_date = "";
						String exec_contract_execution_date = "";
						String exec_contract_end_date = "";
						String exec_contract_notice_period = objects[4].toString();
					...
					//	String user_requested_by_fullname = userService.getUserFullNameById(Integer.parseInt(objects[4].toString()));
						String user_responsible_fullname = objects[8].toString()+" "+objects[9].toString();
						assign_person_mail = objects[10].toString();
						 orga_id = Integer.parseInt(objects[16].toString());

						
						if(objects[7]!=null)
							exec_contract_end_date = sdfOut.format(sdfIn.parse(objects[7].toString()));
						//String assigned_by = session.getAttribute("fullName").toString();
						email_body += "<tr>" + "<td><a href='" + project_url + "updateTab?exec_contra_id=" + contract_id + "'>"+ contract_id + "</a></td>" 
								+ "<td>" + orga_name + "</td>" 
								+ "<td>" + loca_name + "</td>"
								+ "<td>" + dept_name + "</td>" 
								...
								+ "<td>" + exec_contract_date + "</td>" 
								+ "<td>" + exec_contract_execution_date + "</td>" 
								+ "<td>" + exec_contract_notice_period + "</td>" 
								//+ "<td>" + assigned_by + "</td>" 
								+ "<td>" + user_responsible_fullname + "</td>"
								+ "<td>" + exec_contract_end_date + "</td>" 
								+ "</tr>";
					...
					//get mail ids of all admin
					ArrayList<String> email = new ArrayList<String>(); 
					List<Object> mail_idCC = schedulerService.getAllEntityHeadByOrgaId(orga_id);
					Iterator<Object> iterator1 = mail_idCC.iterator();
					while (iterator1.hasNext()) {
						Object object[] = (Object[]) iterator1.next();
						if(object[1] != null)
					...
					sendMail("Executed Contract Assigned", email_body, address,addressCC, contract_id);
