...
public class LegalNoticeServiceImpl implements LegalNoticeService {
	...
	public int saveLegalNotice(LegalNotice_Reference legalNotice_Reference, int id, ArrayList<MultipartFile> legal_doc,
		...
		try {
			...
			if(!legalNotice_Reference.getLega_noti_reply_deadline().toString().equals("")){
				legalNotice
				.setLega_noti_reply_deadline(dateFormat.parse(legalNotice_Reference.getLega_noti_reply_deadline()));
			}
			if(!legalNotice_Reference.getLega_noti_reminder_date().toString().equals("")){
				legalNotice
				.setLega_noti_reminder_date(dateFormat.parse(legalNotice_Reference.getLega_noti_reminder_date()));
			}
			...
			if(id > 0){
				...
				while (itr.hasNext()) {
					...
					documents.setLega_doc_last_generated_value_for_notice_id(requestDocument.getReq_doc_last_generated_value_for_req_id());
			...
			for (int i = 0; i < legal_doc.size(); i++) {
				...
				if (file.getSize() > 0) {
					...
					lega_noti_documents.setLega_doc_generated_file_name(
							"D:/"+project_name+"/Documents/LegalNoticeDocuments/" + generatedFileName);
	...
	public List<LegalNotice_Reference> getAllLegalNotice(HttpSession session) {
		try {
			...
			while (iterator.hasNext()) {
				...
				/*
				 * legal_notice.setLega_noti_by_against(objects[10].toString());
				 * legal_notice.setLega_noti_opposite_party(objects[18].toString
				 * ()); legal_notice.setLega_noti_category_id(Integer.parseInt(
				 * objects[11].toString()));
				 */
	...
	@Override
	public void updateLegalNotice(LegalNotice_Reference legalNotice_Reference, ArrayList<MultipartFile> legal_doc,String status,HttpSession session) {
		try {
			SimpleDateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy");
			...
			String originalFileName = null;
			String generatedFileName = null;
			int lastGeneratedValueDoc = legalNoticeDao.getLastGenerateValueForLegalNotice(legalNotice_Reference.getLega_noti_id(), 1);
			...
			for (int i = 0; i < legal_doc.size(); i++) {
				...
				if (file.getSize() > 0) {
					...
					lega_noti_documents.setLega_doc_original_file_name(originalFileName);
					lega_noti_documents.setLega_doc_generated_file_name("D:/"+project_name+"/Documents/LegalNoticeDocuments/" +generatedFileName);
					lega_noti_documents.setLega_doc_last_generated_value_for_notice_id(lastGeneratedValueDoc);
					lega_noti_documents.setLega_doc_created_date(utilitiesService.getCurrentDate());
					lega_noti_documents.setLega_doc_added_by(utilitiesService.getCurrentSessionUserId(httpSession));
					lega_noti_documents.setLega_doc_related_to(1);
	...
	public String searchLegalNotice(String jsonString, HttpSession session) {
		try {
			...
			List<Object> result = legalNoticeDao.searchLegalNotice(lega_noti_entity_id, lega_noti_unit_id,
					lega_noti_function_id, lega_noti_by_against, lega_noti_category_id, lega_noti_assigned_to_id,
	...
	public LegalNotice_Reference getJoinedDetailsLegalNotice(int lega_noti_id) {
		try {
			...
			if(Integer.parseInt(object[16].toString()) != 0){
				ExternalCounsel exe_name = externalCounselService.getExte_CounById(Integer.parseInt(object[16].toString()));
		...
		try {
			...
			for (int i = 0; i < lega_status_doc.size(); i++) {
				...
				if (file.getSize() > 0) {
					...
					lega_noti_documents.setLega_doc_original_file_name(originalFileName);
					lega_noti_documents.setLega_doc_generated_file_name(
							"D:/"+project_name+"/Documents/LegalNoticeStatusDocuments/" + generatedFileName);
					lega_noti_documents.setLega_doc_last_generated_value_for_notice_id(lastGeneratedValueDoc);
					lega_noti_documents.setLega_doc_created_date(utilitiesService.getCurrentDate());
					lega_noti_documents.setLega_doc_added_by(utilitiesService.getCurrentSessionUserId(httpSession));
	...
	public List<LegalNoticeStatusReference> getAllLegalNoticeStatus(int lega_noti_id) {
		try {
			...
			while (iterator.hasNext()) {
				...
				legalNoticeStatusReference.setLega_person_responsible(userService.getUserFullNameById(Integer.parseInt(object[6].toString())));
				}else{
	...
	public List<Object> getUserByLegalNoticeId(int orga_id, int loca_id, int dept_id) {
		try {
			//List<Object> userList =  legalNoticeDao.getUserByLegalNoticeId(orga_id, loca_id, dept_id);
