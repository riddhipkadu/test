...
public class SendMailDaoImpl implements SendMailDao {
	...
	@Override
	public List<Object> getExecutionDetailsByContractId(int exec_contra_id) {
		try {
			String sql = " SELECT orga_name, loca_name, dept_name,exec_contract_division, exec_contract_notice_period, exec_contract_date, exec_contract_execution_date,exec_contract_end_date,u.user_first_name , u.user_last_name as resposibleperson, u.user_email,exec_contract_id,exec_contract_agree_status ,exec_contract_resposible_person,exec_contract_consider_involve,exec_contract_signatories_assign , orga_id "
					...
					+ "  join mst_user u on u.user_id = exec_contract_resposible_person  where exec_contract_id = "+ exec_contra_id;
			
			System.out.println("getExecutionDetailsByContractId" +sql);
			Query query = em.createNativeQuery(sql);
			return query.getResultList();
		} catch (Exception e) {
			e.printStackTrace();
