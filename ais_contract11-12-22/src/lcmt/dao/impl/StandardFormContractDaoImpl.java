...
public class StandardFormContractDaoImpl implements StandardFormContractDao {
	...
	public int checkDependancySFCType(int sfco_type_id) {
		try {
			String sql = " Select  case when std.sfco_type_id > 0 then 1 else 0 END as sfcoType from mst_sfco_type sfco_type "
					+ " Left join mst_standard_form_contract std on sfco_type.sfco_type_id = std.sfco_type_id "
					+ " where sfco_type.sfco_type_id = "+sfco_type_id+ "LIMIT 1";
			Query query = em.createNativeQuery(sql);
