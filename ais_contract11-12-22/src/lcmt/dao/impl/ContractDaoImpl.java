...
public class ContractDaoImpl implements ContractDao {
	...
	public <T> List<T> getJoinedContractDetails(HttpSession session) {
		try {
			...
			if (Integer.parseInt(session.getAttribute("sess_user_role").toString()) != 2 ) {
				sql = "SELECT orga_name, loca_name, dept_name, cont_agreement_name, cont_requested_by_user_id , cont_requested_date,cont_expected_date,u.user_first_name, u.user_last_name as resposibleperson,cont_targetted_date,cont_id,cont_status,cont_added_by,cont_start_date,cont_end_date,cont.cont_type_name,cont_payment,cont_reminder_date,cont_term,cont_nature,cont_surviving_clause,cont_major_clause,cont_damages,cont_instructions,cont_division FROM mst_contracts LEFT join mst_organization on orga_id = cont_orga_id LEFT join mst_location on loca_id = cont_loca_id LEFT join mst_department on dept_id = cont_dept_id LEFT JOIN mst_contract_type cont ON cont.cont_type_id = cont_id LEFT join mst_user u on u.user_id = cont_responsible_user_id order by cont_id ASC";
			}
			// Normal User
			else {
				sql = "SELECT orga_name, loca_name, dept_name, cont_agreement_name, cont_requested_by_user_id , cont_requested_date,cont_expected_date,u.user_first_name, u.user_last_name as resposibleperson,cont_targetted_date,cont_id,cont_status,cont_added_by,cont_start_date,cont_end_date,cont.cont_type_name,cont_payment,cont_reminder_date,cont_term,cont_nature,cont_surviving_clause,cont_major_clause,cont_damages,cont_instructions,cont_division FROM mst_contracts "
						+ " LEFT join mst_organization on orga_id = cont_orga_id LEFT join mst_location on loca_id = cont_loca_id LEFT join mst_department on dept_id = cont_dept_id  LEFT JOIN mst_contract_type cont ON cont.cont_type_id = cont_id  LEFT join mst_user u on u.user_id = cont_responsible_user_id where (cont_responsible_user_id="
						+ user_id + " OR cont_added_by=" + user_id
						+ " OR cont_responsible_user_id IN(select user_id from mst_user where user_report_to=" + user_id
						+ ") OR cont_added_by IN(select user_id from mst_user where user_report_to=" + user_id + ")) order by cont_id ASC";
			}
			if(Integer.parseInt(session.getAttribute("sess_user_role").toString()) == 7) {
				sql = "SELECT distinct orga_name, loca_name, dept_name, cont_agreement_name, cont_requested_by_user_id , cont_requested_date,cont_expected_date,u.user_first_name, u.user_last_name as resposibleperson,cont_targetted_date,cont_id,cont_status,cont_added_by,cont_start_date,cont_end_date,cont.cont_type_name,cont_payment,cont_reminder_date,cont_term,cont_nature,cont_surviving_clause,cont_major_clause,cont_damages,cont_instructions,cont_division FROM mst_contracts LEFT join mst_organization on orga_id = cont_orga_id LEFT join mst_location on loca_id = cont_loca_id LEFT join mst_department on dept_id = cont_dept_id  LEFT JOIN mst_contract_type cont ON cont.cont_type_id = cont_id  LEFT join mst_user u on u.user_id = cont_responsible_user_id"
				/*SELECT orga_name, loca_name, dept_name, cont_agreement_name, cont_requested_by_user_id , cont_requested_date,cont_expected_date,u.user_first_name, u.user_last_name as resposibleperson,cont_targetted_date,cont_id,cont_status,cont_added_by FROM mst_contracts "
				+ " LEFT join mst_organization on orga_id = cont_orga_id "
				+ " LEFT join mst_location on loca_id = cont_loca_id "
				+ " LEFT join mst_department on dept_id = cont_dept_id "
				+ " LEFT join mst_user u on u.user_id = cont_responsible_user_id"*/
				+ " LEFT join cfg_user_entity_mapping ON cfg_user_entity_mapping.umap_orga_id = cont_orga_id "
				+ "  where cfg_user_entity_mapping.umap_user_id = " + user_id + "order by cont_id ASC";
			}
	...
	public <T> List<T> searchContract(String json, HttpSession session) {
		try {
			...
			String cont_division = jsonObject.get("cont_division").toString();
			...
			String sql = "SELECT orga_name, loca_name, dept_name, cont_agreement_name, cont_requested_by_user_id ,cont_requested_date,cont_expected_date,u.user_first_name , u.user_last_name as resposibleperson,cont_targetted_date,cont_id,cont_status,cont_added_by,cont_start_date,cont_end_date,cont.cont_type_name,cont_payment,cont_reminder_date,cont_term,cont_nature,cont_surviving_clause,cont_major_clause,cont_damages,cont_instructions,cont_division FROM mst_contracts "
					+ " join mst_organization on orga_id = cont_orga_id "
					+ " join mst_location on loca_id = cont_loca_id "
					+ " join mst_department on dept_id = cont_dept_id "
					+ " LEFT JOIN mst_contract_type cont ON cont.cont_type_id = cont_id "
			...
			if(!cont_division.equals("NA"))
				sql += " AND cont_division = '"+cont_division+"'";
	...
	@Override
	public String getLatestContractOtherStatusByContId(int contra_id) {
		try {
			String sql = "SELECT chst_status_others FROM trn_contract_history where chst_contract_id = " + contra_id
					+ " ORDER BY chst_id DESC limit 1";
			Query query = em.createNativeQuery(sql);
			if (!query.getResultList().isEmpty()) {
				return query.getResultList().get(0).toString();
	...
	@Override
	public String getContracttypeByContId(int cont_contract_id) {
		try {
			String sql = "SELECT  cont_type_name FROM trn_contract_contract_type left join  mst_contract_type on cont_type_id =cont_contract_type_id where cont_contract_id =" + cont_contract_id;
					/*+ " ORDER BY chst_id DESC limit 1";*/
			Query query = em.createNativeQuery(sql);
			if (!query.getResultList().isEmpty()) {
				return query.getResultList().get(0).toString();
