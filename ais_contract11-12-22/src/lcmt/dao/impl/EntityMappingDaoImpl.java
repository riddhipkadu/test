	...
	//Method Created : Mugdha Chandratre
		...
		public List<Object> getOrganizationByUserId(HttpSession session) {
			try {
				...
				String sql= "SELECT DISTINCT orga_id,orga_name FROM mst_organization JOIN cfg_user_entity_mapping ON umap_orga_id = orga_id WHERE umap_user_id = "+ sess_user_id;
				Query query = em.createNativeQuery(sql);
				return query.getResultList();
			} catch (Exception e) {
				e.printStackTrace();
