...
function validateForm() {
   ...
   // var cont_requested_date 		= $('#cont_requested_date').val();
	...
	var contract_type       		= $("#cont_type_of_contract").val();
	var cont_force_majeure          = $("#cont_force_majeure").val();
	//alert("Total "+contract_type)
    if (cont_orga_id == 0 ) {
   	 
 		$( "#cont_orga_id" ).attr( "data-placement", "top" );
 		//$( "#cont_orga_id" ).attr( "data-color", "red" );
	...
	var values  =  $('input[name="additional_parties"]').map(function(){
		...
		if(contract_type == 0) {
			$( "#cont_type_of_contract" ).attr( "data-placement", "top" );
	 		$( "#cont_type_of_contract" ).attr( "data-content", "Please select Contract type." );
	...
	*/
	...
	if (cont_reminder_date == 0) {
	 ...
	 if (cont_force_majeure == 0 ) {
	   	 
	 		$( "#cont_force_majeure" ).attr( "data-placement", "top" );
	 		//$( "#cont_orga_id" ).attr( "data-color", "red" );
	 		$( "#cont_force_majeure" ).attr( "data-content", "Please enter Force Majeure" );
	 		$('#cont_force_majeure').popover('show');
	 		$("#cont_force_majeure").focus();
	        return false;
	    }
		 else
	     {
