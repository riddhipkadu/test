...
<div class="col-md-10">
				<div class="header_button">
					...
					<h2 style="color:#032BEC;font-size:24px;float:left;">Litigation Request List</h2>
					<!-- <a href="./masters"><img src="images/DashboardIcons/backold.png" class="backButton" width="100px;"></a> -->

				</div>
</div>
				<div class="col-md-2">
					<center><a href="./addLitigationRequest" class="btn btn-primary">Generate Request</a></center>
				</div>
</div>
	...
	<div class="f_form_content">
				...
				<div class="col-md-4">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<select class="form-control" name="req_liti_entity_id" id="req_liti_entity_id">
					...
					<div class="col-md-4">
						<div class="form-group">
							<label for="user_loca_id" class="control-label col-sm-4"
								for="sel1">Unit :</label>
							<div class="col-sm-8">
								<select class="form-control" id="req_liti_unit_id" name="req_liti_unit_id">
					...
					<div class="col-md-4">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<select class="form-control" id="req_liti_function_id" name="req_liti_function_id">
									  <option value="0">--Select--</option>
							     </select>
					...
					<div class="col-md-4">
						<div class="form-group">
							<label for="user_loca_id" class="control-label col-sm-4"
								for="sel1"> Received date from:</label>
							<div class="col-sm-8">
        ...
        <thead>
            <tr style="background:#0B6EC3;color:#fff;">
                <th>Sr.No.</th>
                <th>Received date (Notice/ Court order)</th>
                <!-- <th>Type</th> -->
        ...
        <tbody id="searchResult">
          <c:forEach items="${listLitigationRequest}" var="req" varStatus="loop">
          <c:if test="${req.req_liti_approval_status !='4'}">
					<tr>
						<td>${loop.index+1 }</td>
						<%-- <td>${req.req_liti_category_name}</td>
						<td>${req.req_liti_company_type}</td>
						<td>${req.req_liti_case_ref_no }</td>
						<td>${req.req_liti_case_filing_date_name }</td> --%>
						<td>${req.req_liti_received_date_name }</td>
						<td>By: ${req.req_liti_party_by } <br/>Against: ${req.req_liti_party_against }</td>
						<td>${req.req_liti_des }</td>
						...
						<c:choose>
							<c:when test="${(user_role_id == 1 || user_role_id == 5) && req.req_liti_approval_status=='0'}">
								<%-- <td class="disapprove" id="appDis${req.req_liti_id}"><button type="button"
											...
											<td id="appDis${req.req_liti_id}"><button type="button" style="margin:2px"
											onclick="getReason(${req.req_liti_id},'3')" name='submit'
											value="submit" class="btn btn-danger">
											<i class='fa fa-thumbs-down'></i>&nbsp;Reject</button>
											<a href="./addLitigationByAccept?id=${req.req_liti_id}&type=liti">
							...
							<c:when test="${(user_role_id == 4 || user_role_id == 6) && req.req_liti_approval_status=='0' }">
								<%-- <td class="disapprove" id="appDis${req.req_liti_id}"><button type="button"
											onclick="getReason(${req.req_liti_id},'0')" name='submit'
											value="submit" class="btn btn-primary">
											<i class='fa fa-thumbs-down'></i>&nbsp;Disapprove</button></td> --%>
											...
											<td id="appDis${req.req_liti_id}"><button type="button" style="margin:2px"
											onclick="getReason(${req.req_liti_id},'1')" name='submit'
											value="submit" class="btn btn-danger">
											<i class='fa fa-thumbs-down'></i>&nbsp;Reject</button>
							...
							<c:when test="${(user_role_id == 1 || user_role_id == 5) && req.req_liti_approval_status=='1'}">
								<%-- <td class="disapprove" id="appDis${req.req_liti_id}"><button type="button"
											onclick="getReason(${req.req_liti_id},'0')" name='submit'
											value="submit" class="btn btn-primary">
											<i class='fa fa-thumbs-down'></i>&nbsp;Disapprove</button></td> --%>
											
											<td id="appDis${req.req_liti_id}">
											...
											<i class='fa fa-thumbs-up'></i>&nbsp;Accept</button></a>
											<a href="./listRejectionReason?relatedTo=litigation&id=${req.req_liti_id}">
						    				<button type="button" name="submit" value="submit" class="btn btn-primary" style="margin-bottom: 4px;">
											&nbsp;Reason
											</button></a></td>
							</c:when>
							
							<c:when test="${(req.req_liti_approval_status=='0' || req.req_liti_approval_status=='1') && user_role_id == 3 }">
								<td>Pending</td>							
							</c:when>
							
							<c:when test="${req.req_liti_approval_status == '3'}">
								 <td>Rejected</br>
								 <a href="./listRejectionReason?relatedTo=litigation&id=${req.req_liti_id}">
						    				<button type="button" name="submit" value="submit" class="btn btn-primary" style="margin-bottom: 4px;">
											&nbsp;Reason
							...
							<c:when test="${(user_role_id == 4 || user_role_id == 6) && req.req_liti_approval_status == '1' }">
						...
						<td><input type="button" id="raisequery" onclick="raiseQuery(${req.req_liti_id},'litigation')" value="Raise Query" class="btn btn-success">
						    <br>
						    <a href="./listRaisedQuery?relatedTo=litigation&id=${req.req_liti_id}" style="margin:2px" class="btn btn-primary">List Queries</a>
						 </td>
					</tr>
					</c:if>
					<c:if test="${req.req_liti_approval_status =='4' && req.req_added_by == sessionScope.sess_user_id}">
					...
					<tr>
					   ...
					   <a href="./editLitiRequest?id=${req.req_liti_id}">
					<button type="button" onclick="" name='submit' style="margin:2px"
					value="submit" class="btn btn-success">
