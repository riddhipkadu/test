...
<div class="col-md-12">
				...
				<div class="header_button">
					...
					<h2 style="color:#032BEC;font-size:24px;float:left;">Contract Request List</h2>
					<!-- <a href="./masters"><img src="images/DashboardIcons/backold.png" class="backButton" width="100px;"></a> -->

				</div>
				</div>
				<div class="col-md-2">
				<center><a href="./addContractRequest" class="btn btn-primary">Generate Contract Request</a></center>
				</div>
			</div>
...
<form role="form" class="form-horizontal" method="post" onsubmit="return validateForm();">
			...
			<div class="f_form_content">
				...
				<div class="col-md-4">
						<div class="form-group">
							...
							<div class="col-sm-8">
								 <select class="form-control" name="req_contract_entity_id" id="req_contract_entity_id">
					...
					<div class="col-md-4">
						<div class="form-group">
							<label for="user_loca_id" class="control-label col-sm-4"
								for="sel1">Unit :</label>
							<div class="col-sm-8">
								<select class="form-control" id="req_contract_unit_id" name="req_contract_unit_id">
					...
					<div class="col-md-4">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<select class="form-control" id="req_contract_function_id" name="req_contract_function_id">
									  <option value="0">--Select--</option>
							     </select>
					...
					<div class="col-md-4">
						<div class="form-group">
							<label for="user_loca_id" class="control-label col-sm-4"
								for="sel1"> Contract date from:</label>
							<div class="col-sm-8">
        ...
        <tbody id="searchResult">
          <c:forEach items="${listContractRequest}" var="req" varStatus="loop">
          <c:if test="${req.req_contract_approval_status !='4'}">
					<tr>
						<td>${loop.index+1 }</td>
						<td>${req.req_contract_type_name}</td>
						<%-- <td>${req.req_contract_date}</td> --%>
						<%-- <td>${req.req_contract_name}</td> --%>
						<td> 
						<c:set var="count" scope="page" value="0"/> 
						    <c:forEach items="${req.req_cont_parties}" var="parties" >
						         <c:set var="count" value="${count+1}"/>
						         ${count}) ${parties.cont_party_name}<br>
						    </c:forEach>
						</td>
						<td>${req.req_contract_desc }</td>
						<td>Contract:
						    ...
						    <c:forEach items="${req.req_term_doc_list}" var="docs1" >
						 ...
						 <%-- <c:choose>
							<c:when test="${req.req_contract_approval_status=='0' && (user_role_id == 1 || user_role_id == 4)}">
											
								<td id="appDis${req.req_contract_id}"><button type="button"
											onclick="getReason(${req.req_contract_id},'1')" name='submit' style="margin:2px;"
											value="submit" class="btn btn-danger">
											<i class='fa fa-thumbs-down'></i>&nbsp;Reject</button>
											
											<button type="button"
											onclick="getAccept(${req.req_contract_id},'2')" name='submit' style="margin:2px;"
											value="submit" class="btn btn-success">
											<i class='fa fa-thumbs-up'></i>&nbsp;Accept</button>
								</td>
							</c:when>
							<c:when test="${user_role_id == 3 && req.req_contract_approval_status=='0'}">
							<td>Pending</td>
							</c:when>
							<c:when test="${req.req_contract_approval_status == '1'}">
								 <td>Rejected</td>
							</c:when>
							<c:otherwise>
                                 <td>Accepted</td>
						...
						<c:choose>
							<c:when test="${ (user_role_id == 1 || user_role_id == 5) && req.req_contract_approval_status=='0' }">
								<%-- <td class="disapprove" id="appDis${req.req_liti_id}"><button type="button"
											...
											<td id="appDis${req.req_contract_id}"><button type="button" style="margin:2px"
											onclick="getReason(${req.req_contract_id},'3')" name='submit'
											value="submit" class="btn btn-danger">
											<i class='fa fa-thumbs-down'></i>&nbsp;Reject</button>
											
											<a href="./addContractByAccept?id=${req.req_contract_id}">
							...
							<c:when test="${(user_role_id == 4 || user_role_id == 6) && req.req_contract_approval_status=='0' }">
								<%-- <td class="disapprove" id="appDis${req.req_liti_id}"><button type="button"
											onclick="getReason(${req.req_liti_id},'0')" name='submit'
											value="submit" class="btn btn-primary">
											<i class='fa fa-thumbs-down'></i>&nbsp;Disapprove</button></td> --%>
											...
											<td id="appDis${req.req_contract_id}"><button type="button" style="margin:2px"
											onclick="getReason(${req.req_contract_id},'1')" name='submit'
											value="submit" class="btn btn-danger">
											<i class='fa fa-thumbs-down'></i>&nbsp;Reject</button>
											
											<a href="./addContractByAccept?id=${req.req_contract_id}">
											<button type="button"
											onclick="" name='submit' style="margin:2px"
											value="submit" class="btn btn-success">
											<i class='fa fa-thumbs-up'></i>&nbsp;Accept</button></a>
											</td>
							</c:when>
							
							<c:when test="${ (user_role_id == 1 || user_role_id == 5) && req.req_contract_approval_status=='1'}">
								<%-- <td class="disapprove" id="appDis${req.req_liti_id}"><button type="button"
											onclick="getReason(${req.req_liti_id},'0')" name='submit'
											value="submit" class="btn btn-primary">
											<i class='fa fa-thumbs-down'></i>&nbsp;Disapprove</button></td> --%>
											
											<td id="appDis${req.req_contract_id}">
											...
											<a href="./listRejectionReason?relatedTo=contract&id=${req.req_contract_id}">
							...
							<c:when test="${(req.req_contract_approval_status== '0' || req.req_contract_approval_status == '1') && user_role_id == 3 }">
								<td>Pending</td>							
							</c:when>
							
							<c:when test="${req.req_contract_approval_status == '3'}">
						<td>Rejected <br/>
							<a href="./listRejectionReason?relatedTo=contract&id=${req.req_contract_id}">
						    <button type="button" name='submit' value="submit" class="btn btn-primary" style="margin-bottom: 4px;">
							&nbsp;Reason
							...
							<c:when test="${(user_role_id == 4 || user_role_id== 6) && req.req_contract_approval_status == '1'}">
						<td>Rejected by SPOC<br/>
							...
							&nbsp;Reason
							 ...
							 <c:when test="${user_role_id == 3 && req.req_contract_approval_status == '2'}">
								<td>Accepted<br/>
								<a href="./listContractStatusForNegotiation?id=${req.req_contract_id}" style="margin:2px" class="btn btn-primary">Contract Status</a></td>
							</c:when> 
							<c:otherwise>
                                 <td>Accepted</td>
							</c:otherwise>
						</c:choose>
						<td><input type="button" id="raisequery" onclick="raiseQuery(${req.req_contract_id},'contract')" value="Raise Query" class="btn btn-success" >
						    <br>
						    <a href="./listRaisedQuery?relatedTo=contract&id=${req.req_contract_id}" style="margin:2px" class="btn btn-primary">List Queries</a>
						 </td>
					</tr>
					</c:if>
					<c:if test="${req.req_contract_approval_status =='4' && req.req_added_by == sessionScope.sess_user_id}">
					<tr>
						<td>${loop.index+1 }</td>
						<td>${req.req_contract_type_name}</td>
						<%-- <td>${req.req_contract_date}</td> --%>
						...
						<td><label>Save for Later</label><br/>
					   	<a href="./editContractRequest?id=${req.req_contract_id}">
											<button type="button" onclick="" name='submit' style="margin:2px"
											value="submit" class="btn btn-success">
