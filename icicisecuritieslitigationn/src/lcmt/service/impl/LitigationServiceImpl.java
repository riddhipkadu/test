...
public class LitigationServiceImpl implements LitigationService {
	...
	public int persistLitigation(Litigation litigation, String status, int id, HttpSession session,
			...
			String type, ArrayList<MultipartFile> litigation_doc) {
		try {
			...
			if (organizationService.getOrganizationById(litigation.getLiti_orga_id()) != null) {
				...
				String orga_shrt_name = organizationService.getOrganizationById(litigation.getLiti_orga_id())
						.getOrga_short_name();
			}
			...
			litigation.setLiti_case_title(
					litigation.getLiti_party_by() + " ...." + litigation.getLiti_company_acting_as() + " v/s "
							+ litigation.getLiti_party_against() + " ...." + litigation.getLiti_opp_party_acting_as());
			...
			if (id > 0 && type.equals("liti")) {
				...
				if (liti_doc != null) {
					...
					while (itr.hasNext()) {
						...
						documents.setLdoc_last_generated_value_for_liti_id(
			...
			if (!esc_appear_counsel.equals("") || !esc_internal_resource.equals("") || !esc_law_firm.equals("")
					|| !esc_others.equals("")) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////			
					/*
					 * if (lastGeneratedValueForLitiDoc >= 0) { if (litigation_doc.size() > 0) {
					 * File dir = new File("C:" + File.separator + project_name + File.separator +
					 * "Documents" + File.separator + "LitigationDocuments"); if (!dir.exists())
					 * dir.mkdirs();
					 * 
					 * lastGeneratedValueForLitiDoc++; originalFileName =
					 * litigation_doc.getOriginalFilename(); generatedFileName = "uploadedLitiDoc_"
					 * + ldoc_liti_id + "_" + lastGeneratedValueForLitiDoc + "." +
					 * litigation_doc.getOriginalFilename().split("\\.")[1]; File newFile = new
					 * File(dir.getPath() + File.separator + generatedFileName); if
					 * (!newFile.exists()) { newFile.createNewFile(); }
					 * 
					 * @SuppressWarnings("resource") OutputStream outputStream = new
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				if (Integer.parseInt(object[5].toString()) != 0) {
					litigationDetails.setExte_coun_name(externalCounselService
							.getExte_CounById(Integer.parseInt(object[5].toString())).getExte_coun_name());
				} else {
				...
				if (object[6] != null)
					litigationDetails
							.setLiti_internally_handled_name(object[6].toString() + ' ' + object[7].toString());
				else
				...
				if (Integer.parseInt(object[26].toString()) != 0) {
					litigationDetails.setLiti_assigned_name(
							userService.getUserFullNameById(Integer.parseInt(object[26].toString())));
				} else {
				...
				if (Integer.parseInt(object[42].toString()) != 0) {
					litigationDetails.setLiti_secondary_responsible_name(
							userService.getUserFullNameById(Integer.parseInt(object[42].toString())));
				} else {
					litigationDetails.setLiti_secondary_responsible_name("NA");
				}

				if (Integer.parseInt(object[43].toString()) != 0) {
					litigationDetails.setLiti_third_responsible_name(
							userService.getUserFullNameById(Integer.parseInt(object[43].toString())));
				} else {
					litigationDetails.setLiti_third_responsible_name("NA");
				}

				if (Integer.parseInt(object[44].toString()) != 0) {
					litigationDetails.setLiti_fourth_responsible_name(
							userService.getUserFullNameById(Integer.parseInt(object[44].toString())));
				} else {
					litigationDetails.setLiti_fourth_responsible_name("NA");
				}

				if (object[45] != null)
					litigationDetails.setLiti_pay(object[45].toString());
				else
					litigationDetails.setLiti_pay("NA");

				if (object[46] != null)
					litigationDetails.setLiti_part_acm(object[46].toString());
				else
					litigationDetails.setLiti_part_acm("NA");

				if (object[47] != null)
					litigationDetails.setLiti_acm_note_date(sdfOut.format(sdfIn.parse(object[47].toString())));
				else
					litigationDetails.setLiti_acm_note_date("NA");
	...
	public int saveHearingStage(HearingStage hearingStage, ArrayList<MultipartFile> hearing_doc,
		...
		try {
			...
			if (!status.equals("Draft")) {
				...
				// Stages stageName =
				// stageService.getStagesById(hearingStage.getTtrn_stage_id());
			...
			email_body += "<table style='width:80%;' border='1'>" + "<thead>"
					+ "<tr style='background:#0B6EC3;color:#fff;'>" + "<th>Litigation ID</th>"
					+ "<th>Next hearing date</th>" + "<th>Responsible person</th>" + "<th>Stage description</th>"
					+ "<th>Action item due date</th>" + "</tr>" + "</thead>" + "<tbody>";
			...
			email_body += "<tr>" + "<td><a href='" + project_url + "litigationDetails?liti_id=" + litigation_id + "'>"
					+ litigation_id + "</a></td>" + "<td>" + next_hearing_date + " </td>" + "<td>" + responsible_person
					+ "</td>" + "<td>" + stage_desc + "</td>" + "<td>" + action_item_due_dt + "</td>" + "</tr>";
			...
			for (int i = 0; i < hearing_doc.size(); i++) {
				...
				if (file.getSize() > 0) {
					File dir = new File("C:" + File.separator + project_name + File.separator + "Documents"
							+ File.separator + "HearingStageDocuments");
					if (!dir.exists())
						dir.mkdirs();
	...
	public String getHearingDetailsByLitiId(int liti_id, HttpSession session) {
		try {
			...
			while (iterator.hasNext()) {
				...
				List<Object> res_coun = litigationDao
						.getCounselByHearingStageId(Integer.parseInt(objects[5].toString()));
				if (res_coun != null) {
			...
			if (Integer.parseInt(session.getAttribute("sess_user_role").toString()) == 1
					|| Integer.parseInt(session.getAttribute("sess_user_role").toString()) == 5) {
				...
				while (iterator1.hasNext()) {
					...
					JSONArray sendCoun = new JSONArray();
					List<Object> res_coun = litigationDao
							.getCounselByHearingStageId(Integer.parseInt(object[5].toString()));
					if (res_coun != null) {
						Iterator<Object> coun = res_coun.iterator();
						while (coun.hasNext()) {
							Object[] obj = (Object[]) coun.next();
	...
	public int updateHearingStage(HearingStage hearingStage, ArrayList<MultipartFile> hearing_doc,
		...
		try {
			...
			if ((oldData.getTtrn_stage_status().equals("Draft")
					|| oldData.getTtrn_stage_status().equals("Waiting for Approval")) && status.equals("Pending"))
			...
			if ((oldData.getTtrn_stage_status().equals("Draft")
					|| oldData.getTtrn_stage_status().equals("Waiting for Approval")) && !status.equals("Draft")) {
				// persist litigation logs
				ActivityLogs logs = new ActivityLogs();
				logs.setLog_activity_id(hearingStage.getTtrn_litigation_id());
			...
			if (status.equals("Pending") && (!oldData.getTtrn_stage_status().equals("Draft")
					|| !oldData.getTtrn_stage_status().equals("Waiting for Approval"))) {
				// persist litigation logs
				ActivityLogs logs = new ActivityLogs();
				logs.setLog_activity_id(hearingStage.getTtrn_litigation_id());
			...
			// send mail to admin and number of mail ids which enter in litigation are
			// marked in cc
	...
	public void updateLitigation(Litigation litigation, String status, HttpSession session,
		...
		try {
			...
			// litigation.setLiti_client_liti_id("L" + (String.format("%02d",
			// litigation.getLiti_dept_id())
			// + String.format("%07d",
			...
			litigation.setLiti_updated_at(utilitieservice.getCurrentDateWithTime());
			litigation.setLiti_updated_by(Integer.parseInt(session.getAttribute("sess_user_id").toString()));
	...
	public String searchLitigation(String json, HttpSession session) {
		try {
			...
			while (iterator.hasNext()) {
				...
				jsonObject.put("liti_assigned_to", userService.getUserFullNameById(litigation.getLiti_assigned_to()));
	...
	public void saveLitigationCompletion(Litigation litigation, ArrayList<MultipartFile> liti_completion_doc) {
		try {
			...
			// dis:"+litigation.getLiti_disposal_date()+"
			// synop:"+litigation.getLiti_synopsis_of_order()+"
			// cou:"+litigation.getLiti_completion_court_id()+"
			// filing:"+litigation.getLiti_last_date_for_filling_appeal()+"
			// comm:"+litigation.getLiti_comments());
			...
			if (litigation.getLiti_last_date_for_filling_appeal() != null
					&& !litigation.getLiti_last_date_for_filling_appeal().equals(""))
			...
			email_body += "<table style='width:80%;' border='1'>" + "<thead>"
					+ "<tr style='background:#0B6EC3;color:#fff;'>" + "<th>Litigation ID</th>" + "<th>Result</th>"
					+ "<th>Synopsis of order</th>"
					// + "<th>Court</th>"
					+ "<th>Filing appeal last date</th>" + "<th>Comments</th>" + "</tr>" + "</thead>" + "<tbody>";
			...
			email_body += "<tr>" + "<td><a href='" + project_url + "litigationDetails?liti_id=" + litigation_id + "'>"
					+ litigation_id + "</a></td>" + "<td>" + result + " </td>" + "<td>" + synopsis_order + "</td>"
					+ "<td>" + case_filing_date + "</td>" + "<td>" + comments + "</td>" + "</tr>";
		...
		try {
			...
			for (int i = 0; i < counsel_agreed_doc.size(); i++) {
				...
				if (lastGeneratedValueForFeesDoc >= 0) {
					if (file.getSize() > 0) {
						File dir = new File("C:" + File.separator + project_name + File.separator + "Documents"
								+ File.separator + "LitigationFeesDocuments" + File.separator + "CounselFeesDocuments"
								+ File.separator + "CounselFeesAgreedDocuments");
						if (!dir.exists())
							dir.mkdirs();
						...
						litigationDocuments.setFdoc_generated_file_name("C:/" + project_name
								+ "/Documents/LitigationFeesDocuments/CounselFeesDocuments/CounselFeesAgreedDocuments/"
								+ generatedFileName);
	...
	public List<CounselFeesDetails> getCounselFeesByLitiId(int liti_id) {
		try {
			...
			while (iterator.hasNext()) {
				...
				counselFeesDetails.setFees_doc(litigationDocumentsDao.getAllDocumentByFeesId(
						LitigationFeesDocuments.class, Integer.parseInt(objects[0].toString()), "AAC"));// AAC - Agreed
																										// Amount
																										// Counsel
		...
		try {
			...
			for (int i = 0; i < advocate_agreed_doc.size(); i++) {
				...
				if (lastGeneratedValueForFeesDoc >= 0) {
					if (file.getSize() > 0) {
						File dir = new File("C:" + File.separator + project_name + File.separator + "Documents"
								+ File.separator + "LitigationFeesDocuments" + File.separator + "AdvocateFeesDocuments"
								+ File.separator + "AdvocateFeesAgreedDocuments");
						if (!dir.exists())
							dir.mkdirs();
						...
						litigationDocuments.setFdoc_original_file_name(originalFileName);
						litigationDocuments.setFdoc_generated_file_name("C:/" + project_name
								+ "/Documents/LitigationFeesDocuments/AdvocateFeesDocuments/AdvocateFeesAgreedDocuments/"
								+ generatedFileName);
						litigationDocuments.setFdoc_last_generated_value_for_fees_id(lastGeneratedValueForFeesDoc);
	...
	public List<AdvocateFeesDetails> getAdvocateFeesDetailsByLitiId(int liti_id) {
		try {
			...
			while (iterator.hasNext()) {
				...
				advocateFeesDetails.setFees_doc(litigationDocumentsDao.getAllDocumentByFeesId(
						LitigationFeesDocuments.class, Integer.parseInt(objects[7].toString()), "AAA"));
	...
	public void updateAdvocateFees(LitigationAdvocateFees advocateFees, ArrayList<MultipartFile> advocate_agreed_doc) {
		try {
			LitigationAdvocateFees OldadvocateFees = litigationDao
					.getAdvocateFeesDetailsById(advocateFees.getLadv_id());
	...
	public List<LitigationReference> getAllListLitigation(HttpSession session) {
		try {
			...
			/*
			 * String User_name = (session.getAttribute("us_full_name").toString());
			 * System.out.println("user_id "+user_id);
			 * System.out.println("User_name "+User_name);
			 */
			String user_name = litigationDao.getUserFullName(user_id);
			System.out.println("User_name " + user_name);
			...
			if (result != null) {
				...
				while (iterator.hasNext()) {
					...
					next_hearing_date = litigationDao.getLatestNextHearingStageDate(Integer.parseInt(object[0].toString()));
					if (!next_hearing_date.equals("NA")) {
						litigationReference.setLiti_next_hearing_date(sdfOut.format(sdfIn.parse(next_hearing_date)));
					} else {
						litigationReference.setLiti_next_hearing_date("NA");
					}
					// System.out.println("hearing date :"+sdfIn.parse(next_hearing_date)+" todays
					// date :"+new Date());
					...
					if (Integer.parseInt(object[5].toString()) != 0) {
						litigationReference.setLiti_external_counsel(externalCounselService
								.getExte_CounById(Integer.parseInt(object[5].toString())).getExte_coun_name());
					} else {
					...
					if (object[38] != null && object[37] != null)
						litigationReference.setLiti_updated_by("Edited By" + " " + object[38].toString() + "   "
					...
					/*
					 * if (object[37] != null)
					 * litigationReference.setLiti_updated_at(sdfOut.format(sdfIn.parse(object[37].
					 * toString()))); else litigationReference.setLiti_updated_at("NA");
					 */
	...
	public int savePaidCounselFees(CounselPaidFees fees, ArrayList<MultipartFile> counsel_paid_doc) {
		try {
			LitigationCounselFees counselFees = litigationDao
					.getCounselFeesDetailsById(fees.getCpaid_counsel_fees_id());
			...
			for (int i = 0; i < counsel_paid_doc.size(); i++) {
				...
				if (lastGeneratedValueForFeesDoc >= 0) {
					if (file.getSize() > 0) {
						File dir = new File("C:" + File.separator + project_name + File.separator + "Documents"
								+ File.separator + "LitigationFeesDocuments" + File.separator + "CounselFeesDocuments"
								+ File.separator + "CounselFeesPaidDocuments");
						if (!dir.exists())
							dir.mkdirs();
						...
						litigationDocuments.setFdoc_original_file_name(originalFileName);
						litigationDocuments.setFdoc_generated_file_name("C:/" + project_name
								+ "/Documents/LitigationFeesDocuments/CounselFeesDocuments/CounselFeesPaidDocuments/"
								+ generatedFileName);
						litigationDocuments.setFdoc_last_generated_value_for_fees_id(lastGeneratedValueForFeesDoc);
	...
	public List<CounselFeesDetails> getPaidFeesDetailsByConFeesId(int counsel_fees_id) {
		try {
			...
			for (CounselPaidFees counselPaidFees : res) {
				...
				details.setFees_doc(litigationDocumentsDao.getAllDocumentByFeesId(LitigationFeesDocuments.class,
						counselPaidFees.getCpaid_id(), "PC"));
	...
	public int savePaidAdvocateFees(AdvocatePaidFees fees, ArrayList<MultipartFile> advocate_paid_doc) {
		try {
			LitigationAdvocateFees advocateFees = litigationDao
					.getAdvocateFeesDetailsById(fees.getApaid_advocate_fees_id());
			...
			for (int i = 0; i < advocate_paid_doc.size(); i++) {
				...
				if (lastGeneratedValueForFeesDoc >= 0) {
					if (file.getSize() > 0) {
						File dir = new File("C:" + File.separator + project_name + File.separator + "Documents"
								+ File.separator + "LitigationFeesDocuments" + File.separator + "AdvocateFeesDocuments"
								+ File.separator + "AdvocateFeesPaidDocuments");
						if (!dir.exists())
							dir.mkdirs();
						...
						litigationDocuments.setFdoc_original_file_name(originalFileName);
						litigationDocuments.setFdoc_generated_file_name("C:/" + project_name
								+ "/Documents/LitigationFeesDocuments/AdvocateFeesDocuments/AdvocateFeesPaidDocuments/"
								+ generatedFileName);
						litigationDocuments.setFdoc_last_generated_value_for_fees_id(lastGeneratedValueForFeesDoc);
	...
	public List<AdvocateFeesDetails> getPaidFeesDetailsByAdvocateFeesId(int advocate_fees_id) {
		try {
			...
			for (AdvocatePaidFees AdvocatePaidFees : res) {
				...
				details.setFees_doc(litigationDocumentsDao.getAllDocumentByFeesId(LitigationFeesDocuments.class,
						AdvocatePaidFees.getApaid_id(), "PA"));
				sendList.add(details);
			}
			return sendList;
	...
	public int isAdvocatePaidFeesGreaterThanAgreedFees(int apaid_fees_amount, int apaid_advocate_fees_id) {
		try {
			int result = litigationDao.isAdvocatePaidFeesGreaterThanAgreedFees(apaid_fees_amount,
	...
	public int deleteHearingStageDoc(int doc_id) {
		try {
			...
			// litigationDocumentsDao.getHearingDocumentFilePath(doc_id);
	...
	public int deleteHearingCounselById(String json) {
		try {
			...
			/*
			 * int res = litigationDao.deleteHearingCounselById(hsco_id); JSONObject send =
			 * new JSONObject(); send.put("status",res); if(res==1) { send.put("msg",
			 * "Counsel deleted succesfully."); }else{ send.put("msg", "Please try again.");
	...
	public int saveHearingStageOnHearing(HearingStage hearingStage, ArrayList<MultipartFile> hearing_doc, String status,
		...
		try {
			...
			HearingStageOnHearing hearingStatus = litigationDao
					.getHearingOnHearingDetailsById(Integer.parseInt(hearing_id));
			...
			email_body += "<p style='text-align:justify;width:70%;'>The following Hearing stage is been created against litigation and waiting for your approval. You are requested to follow the link by clicking on the 'ID'.</p>"
					+ "<h2 style='font-size:16px;font-weight:bold;'>Hearing stage details :</h2>";
			email_body += "<table style='width:80%;' border='1'>" + "<thead>"
					+ "<tr style='background:#0B6EC3;color:#fff;'>" + "<th> ID</th>" + "<th>Next hearing date</th>"
					// + "<th>Responsible person</th>"
					+ "<th>Stage description</th>" + "<th>Action item due date</th>" + "<th>Action item</th>" + "</tr>"
					+ "</thead>" + "<tbody>";
			String litigation_id = "";
			...
			email_body += "<tr>" + "<td><a href='" + project_url + "litigationDetails?liti_id=" + litigation_id + "'>"
					+ litigation_id + "</a></td>" + "<td>" + next_hearing_date + " </td>" + "<td>" + stage_desc
					+ "</td>" + "<td>" + action_item_due_dt + "</td>" + "<td>" + action_item + "</td>" + "</tr>";
			...
			/*
			 * Litigation liti =
			 * litigationDao.getLitigationByLitiId(hearingStage.getTtrn_litigation_id());
			 * 
			 * String liti_Allmail_id = liti.getLiti_mail_id_cc(); if(liti_Allmail_id !=
			 * null && (!liti_Allmail_id.equals(""))){ String[] mail_id =
			 * liti_Allmail_id.split(","); InternetAddress[] addressCC = new
			 * InternetAddress[mail_id.length]; try { if(mail_id.length != 0){ for(int i =
			 * 0; i< mail_id.length ; i++){ addressCC[i] = new InternetAddress(mail_id[i]);
			 * } } } catch (AddressException e) { e.printStackTrace(); }
			 * sendMailService.sendMail(email_subject, email_body, address, addressCC,
			 * litigation_id); }
			 */
			sendMailService.sendMail(email_subject, email_body, address, address, litigation_id);
	...
	public String acmLitigation(String json, HttpSession session) {
		...
		try {
			...
			File dir1 = new File(
					"C:"+ File.separator + project_name + File.separator + "ACMReport");
			...
			Object generatedExcelFile = "Litigation_ACMReport_" +test+ ".xls";
			...
			HSSFSheet sheet = workbook.createSheet("Litigation ACM Report");
			HSSFRow rowhead0 = sheet.createRow(0);
			rowhead0.createCell(0).setCellValue("Particulars");
			
			HSSFRow rowhead = sheet.createRow(1);
			rowhead.createCell(1).setCellValue("Cases less than Rs 10 Lakhs");
			rowhead.createCell(2).setCellValue("Cases greater than Rs 10 Lakhs");
			rowhead.createCell(3).setCellValue("Total");
		
			
			  List<Object> res = litigationDao.acmLitigation(json,session);
				Iterator<Object> iterator = res.iterator();
				...
				while (iterator.hasNext()) {
					...
					row2.createCell(0).setCellValue("Total No. of cases");
					row2.createCell(1).setCellValue(objects[0].toString());
					row2.createCell(2).setCellValue(objects[1].toString());
					row2.createCell(3).setCellValue(objects[4].toString());
					
					HSSFRow row3 = sheet.createRow(3);
					row3.createCell(0).setCellValue("Amount");
					row3.createCell(1).setCellValue(objects[2].toString());
					row3.createCell(2).setCellValue(objects[3].toString());
					row3.createCell(3).setCellValue(objects[5].toString());
			
					HSSFRow row4 = sheet.createRow(4);
					row4.createCell(0).setCellValue("No. of Cases with possible liability and where provisions are held");
					row4.createCell(1).setCellValue(objects[6].toString());
					row4.createCell(2).setCellValue(objects[7].toString());
					row4.createCell(3).setCellValue(objects[8].toString());
					
					HSSFRow row5 = sheet.createRow(5);
					row5.createCell(0).setCellValue("Amount of possible liability and which are specified as contingent liability");
					row5.createCell(1).setCellValue(objects[9].toString());
					row5.createCell(2).setCellValue(objects[10].toString());
					row5.createCell(3).setCellValue(objects[11].toString());
					
					HSSFRow row6 = sheet.createRow(6);
					row6.createCell(0).setCellValue("Cases filed by the Company and cases adjudged remote for which no provisions are taken nor regarded as contingent liability ");
					row6.createCell(1).setCellValue(objects[12].toString());
					row6.createCell(2).setCellValue(objects[13].toString());
					row6.createCell(3).setCellValue(objects[14].toString());
	...
	public void downloadACMReport(HttpServletResponse response) {
		try {
			...
			String path ="C:/icicisecuritieslitigation/ACMReport";
