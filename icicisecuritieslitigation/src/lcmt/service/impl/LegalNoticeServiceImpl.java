	...
	public List<LegalNotice_Reference> getAllLegalNotice(HttpSession session) {
		try {
			...
			while (iterator.hasNext()) {
				...
				/*
				 * if (objects[13] != null) {
				 * legal_notice.setLega_noti_secondary_responsible_person_name(
				 * objects[13].toString() + " " + objects[9].toString()); } else {
				...
				if (Integer.parseInt(objects[13].toString()) != 0) {
					legal_notice.setLega_noti_secondary_responsible_person_name(
							userService.getUserFullNameById(Integer.parseInt(objects[13].toString())));
				} else {
					legal_notice.setLega_noti_secondary_responsible_person_name("NA");
				}
				
				if (Integer.parseInt(objects[14].toString()) != 0) {
					legal_notice.setLega_noti_third_responsible_person_name(
							userService.getUserFullNameById(Integer.parseInt(objects[14].toString())));
				} else {
					legal_notice.setLega_noti_third_responsible_person_name("NA");
				}
