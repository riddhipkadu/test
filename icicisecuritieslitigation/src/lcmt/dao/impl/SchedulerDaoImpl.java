...
public class SchedulerDaoImpl implements SchedulerDao {
	...
	public List<Object> getAssignPersonFromReminderDate() {
		try {
			String sql = "SELECT distinct quer_assigned_to FROM mst_query WHERE quer_reminder_date = CONVERT(date,GETDATE()) AND quer_status !='Draft'";
	...
	public List<Object> getReminderQueryMailToAssignPerson(int assign_to) {
		try {
			String sql = "SELECT orga_name,loca_name,dept_name,assin.user_first_name, assin.user_last_name as assigned_to,assin.user_email as ass_user_mail, quer_from_id ,quer.quer_query_date,quer.quer_reply_date,quer.quer_reminder_date, quer.quer_id, quer.quer_query FROM mst_query quer "
					...
					+ " JOIN mst_user assin ON assin.user_id = quer.quer_assigned_to WHERE quer_reminder_date = CONVERT(date,GETDATE())"
	...
	public List<Object> getAssignPersonFromAlertDate() {
		try {
			String sql = "select distinct ttrn_responsible_person from trn_hearing_stages where ttrn_stage_status!='Draft' AND ttrn_first_alert = CONVERT(date,GETDATE()) or ttrn_second_alert = CONVERT(date,GETDATE()) ";
			Query query = em.createNativeQuery(sql);
			return query.getResultList();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Object> getAlertHearingStageMailToAssignPerson(int resp_person_id) {
		try {
			String sql = "SELECT ttrn_next_hearing_date,ttrn_first_alert,ttrn_second_alert,ttrn_third_alert,ttrn_stage_description,ttrn_hearing_stage,ttrn_responsible_person, user_first_name, user_last_name as responsiblePerson, user_email , ttrn_litigation_id FROM trn_hearing_stages Join mst_user on user_id = ttrn_responsible_person"
					...
					+ " AND ttrn_stage_status!='Draft' AND (ttrn_first_alert = CONVERT(date,GETDATE()) OR ttrn_second_alert = CONVERT(date,GETDATE()) OR ttrn_third_alert = CONVERT(date,GETDATE()))";
			Query query = em.createNativeQuery(sql);
			return query.getResultList();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Object> getAssignPersonLegalNoticeFromReminderDate() {
		try {
			String sql = "select distinct lega_noti_assigned_to_id from mst_legal_notice where lega_noti_reminder_date = CONVERT(date,GETDATE()) AND lega_noti_status !='Draft' ";
			Query query = em.createNativeQuery(sql);
			return query.getResultList();
	...
	public List<Object> getReminderLegalNoticeMailToAssignPerson(int matter_handled_by_id) {
		try {
			String sql = "SELECT orga.orga_name,loca.loca_name,dept.dept_name,leg_cat.lega_noti_category_name,lega_noti.lega_noti_id,CONVERT(varchar(12),lega_noti.lega_noti_dated,105) lega_noti_dated,CONVERT(varchar(12),lega_noti.lega_noti_recived_on,105) lega_noti_recived_on,CONVERT(varchar(12),lega_noti.lega_noti_reply_deadline,105) lega_noti_reply_deadline, usr.user_first_name, usr.user_last_name as fullname, usr.user_email as email, CONVERT(varchar(12),lega_noti.lega_noti_reminder_date,105) lega_noti_reminder_date FROM mst_legal_notice lega_noti JOIN mst_organization orga ON orga.orga_id = lega_noti.lega_noti_entity_id  "
					...
					+ " where lega_noti.lega_noti_reminder_date = CONVERT(date,GETDATE()) AND lega_noti.lega_noti_assigned_to_id = "
	...
	public List<Object> getAssignedPersonLegalnoticeStatus() {
		try {
			String sql = "select DISTINCT lega_person_responsible from trn_legal_notice_status where lega_reminder_date =  CONVERT(date,GETDATE()) AND lega_notice_status !='Draft'";
	...
	public List<Object> getReminderLegalNoticeStatusMailToAssignPerson(int id) {
		try {
			String sql = "SELECT orga.orga_name,loca.loca_name,dept.dept_name,lega_noti.lega_noti_id,lega_noti.lega_noti_reply_deadline,usr.user_first_name,usr.user_last_name, usr.user_email,lega_status.lega_action_taken,lega_status.lega_next_action_item,lega_status.lega_action_item_due_date,lega_status.lega_reminder_date,lega_status.lega_received_date,lega_status.lega_status_id FROM mst_legal_notice lega_noti JOIN mst_organization orga ON orga.orga_id = lega_noti.lega_noti_entity_id  "
					...
					+ " where lega_status.lega_reminder_date = CONVERT(date,GETDATE()) AND lega_status.lega_person_responsible ="+id;
	...
	public List<Object> getAssignPersonTaskCompletionFromReminderDate() {
		try {
			String sql = "Select distinct atrn_resposible_user from ttrn_action_item_transactional where atrn_status = 'Pending' and CONVERT(date,GETDATE()) = (atrn_action_due_date - interval atrn_first_alert_prior_days day) or CONVERT(date,GETDATE()) = (atrn_action_due_date - interval atrn_second_alert_prior_days day); ";
			Query query = em.createNativeQuery(sql);
			return query.getResultList();
	...
	public List<Object> getReminderTaskCompletionMailToAssignPerson(int task_assigned_id) {
		...
		String sql = " select orga_name,loca_name,dept_name,user_first_name, user_last_name, user_email, atrn_resposible_user, exec_action_id, atrn_action_due_date, atrn_frequency, exec_action_item from ttrn_action_item_transactional "
					...
					+ " where atrn_resposible_user = "+task_assigned_id+" and atrn_status = 'Pending' and (CONVERT(date,GETDATE()) = (atrn_action_due_date - interval atrn_first_alert_prior_days day) or CONVERT(date,GETDATE()) = (atrn_action_due_date - interval atrn_second_alert_prior_days day)) ";
		Query query = em.createNativeQuery(sql);
		return query.getResultList();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Object> getAssignPersonFrom3rdAlertDate() {
		try {
			String sql = "select distinct ttrn_hearing_stage from trn_hearing_stages where ttrn_stage_status!='Draft' AND ttrn_third_alert = CONVERT(date,GETDATE()) ";
			Query query = em.createNativeQuery(sql);
			return query.getResultList();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Object> get3rdAlertHearingStageMailToAssignPerson(int matter_handled_by_id) {
		try {
			String sql = "SELECT ttrn_next_hearing_date,ttrn_first_alert,ttrn_second_alert,ttrn_third_alert,ttrn_stage_description,ttrn_hearing_stage,ttrn_responsible_person, user_first_name, user_last_name as responsiblePerson, user_email , ttrn_litigation_id, liti.liti_case_title, liti.liti_court, court.court_name FROM trn_hearing_stages "
					...
					+ " where ttrn_hearing_stage = "+ matter_handled_by_id;
			Query query = em.createNativeQuery(sql);
			return query.getResultList();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Object> getHearingStageOnHearing() {
		try {
			String sql = "SELECT ttrn_litigation_id,ttrn_next_hearing_date, ttrn_first_alert,ttrn_second_alert,ttrn_third_alert,ttrn_stage_description,ttrn_hearing_stage,ttrn_responsible_person FROM trn_hearing_stages "
					+ " where ttrn_stage_status!='Draft' AND ttrn_next_hearing_date = CONVERT(date,GETDATE())";
			Query query = em.createNativeQuery(sql);
			return query.getResultList();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Object> getAssignPersonFromAlertSarfaesiactDate() {
		...
		try {
			String sql = "select distinct sarf_act_id from mst_sarfaesiact  where sarf_first_reminder = CONVERT(date,GETDATE()) or sarf_second_reminder = CONVERT(date,GETDATE())";
			Query query = em.createNativeQuery(sql);
	...
	public List<Object> getAlertSarfaesiactMailToExecutor(int sarf_act_id) {
		// TODO Auto-generated method stub
		try {
			String sql = "select sarf_act_id ,sarf.sarf_loan_no,sarf.sarf_borrower,sarf.sarf_security_type,sarf.sarf_security_Interest,sarf.sarf_npa_date,sarf.sarf_filling_date,sarf.sarf_NotiIssue_date,sarf.sarf_Executor_id,user.user_email as email,sarf.sarf_first_reminder,sarf.sarf_second_reminder from mst_sarfaesiact sarf JOIN mst_user user ON user.user_id = sarf.sarf_Executor_id"
					+ " where sarf.sarf_act_id = " + sarf_act_id;
	...
	public List<Object> getAssignPersonFromAlertSarfaesiActionDate() {
		// TODO Auto-generated method stub
		try {
		String sql="select distinct sarf_action_id from mst_sarfaesiaction where action_first_remind_date = CONVERT(date,GETDATE())";
		Query query=em.createNativeQuery(sql);
		return query.getResultList();
		} catch (Exception e) {
	...
	public List<Object> getAlertSarfaesiActionMailToExecutor(int sarf_action_id) {
		...
		String sql="select act.sarf_action_id,act.sarf_action_status,act.sarf_action_item,act.sarf_nextaction_item,"
				+ "act.sarf_action_NotiIssue_date,act.sarf_action_due_date,act.action_first_remind_date,sarf.sarf_Executor_id"
				+ " from mst_sarfaesiaction act join mst_sarfaesiact sarf  ON sarf.sarf_act_id = act.action_sarf_act_id "
				+ "where act.sarf_action_id="+ sarf_action_id ;
