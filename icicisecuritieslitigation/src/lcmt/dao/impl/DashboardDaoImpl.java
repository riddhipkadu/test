...
public class DashboardDaoImpl implements DashboardDao {
	...
	public List<Object> getOverallLitigationStatusGraph(HttpSession session) {
		try {
			...
			String sql = "SELECT COUNT(CASE WHEN liti_litigation_result='Pending' then 1 ELSE NULL END ) as Pending, COUNT(CASE WHEN liti_litigation_result='Against' then 1 ELSE NULL END ) as Against, COUNT(CASE WHEN liti_litigation_result='In Favour' then 1 ELSE NULL END ) as InFavour, COUNT(CASE WHEN liti_litigation_result='Settled' then 1 ELSE NULL END ) as Settled, COUNT(CASE WHEN liti_litigation_result='Withdraw' then 1 ELSE NULL END ) as Withdraw FROM mst_litigation ";
			if(user_role != 1){
				sql += " where liti_added_by ="+user_id+ " OR liti_assigned_to ="+user_id;
			}
			Query query = em.createNativeQuery(sql);
			
			System.out.println("Dashbaord pie chart  "+sql);
	...
	public List<Object> getAllLitigationForCategory(int liti_type_id, HttpSession session) {
		try {
			...
			System.out.println("getAllLitigationForCategory  " +sql);
			return query.getResultList();
		} catch (Exception e) {
			e.printStackTrace();
		}
