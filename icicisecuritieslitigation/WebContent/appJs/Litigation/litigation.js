...
/*--------------------------------------------------------------------------------------------------------------------------------*/
	...
	$('#to_acm_date').datepicker({ autoclose: true}).datepicker('setStartDate', e.date).focus();
	});
	
	
	$("#to_acm_date").datepicker({
...
/*---------------------------------------------------------------------------------------------------------------------------------------*/
	...
	$("#searchLitigation").click(function(){
		...
		var liti_secondary_responsible = $("#liti_secondary_responsible").val();
		var liti_third_responsible_person = $("#liti_third_responsible_person").val();
		//alert(liti_assigned_to);
		...
		data["liti_litigation_result"]     = liti_litigation_result;
		...
		//alert(liti_assigned_to);
		data["liti_secondary_responsible"] = liti_secondary_responsible;
	   // alert(liti_secondary_responsible);
		data["liti_third_responsible_person"] = liti_third_responsible_person;
		//alert(liti_third_responsible_person);
		...
		$.ajax({
			...
			success : function(result){
				...
				$.each(result,function(key,value){
					...
					data += "<td>"+value['liti_litigation_result']+"</td>";
					data += "<td>"+value['liti_assigned_to']+"</td>"
					data += "<td>"+value['liti_secondary_responsible']+"</td>"
					data += "<td>"+value['liti_third_responsible_person']+"</td>"
					data +=" <td class='edit'>";
		...
		$.ajax({
			...
			success : function(response) {
				 ...
				 if(response.message=="success"){
					 downloadURI(response.xl_liti_url, "Litigation_Report.xlsx");
				 }else{
			...
			error : function(xhr, ajaxOptions, thrownError) {
				$("#loader").hide();	
				$('#errorexport').html("There is error:" + thrownError);
			},
		});
...
$("#liti_dept_id").change(function(){
	...
	if (user_dept_id > 0 && user_loca_id > 0 && user_dept_id > 0) {
		...
		var jsonString = JSON.stringify(data);
				...
				$.ajax({
					...
					success : function(userList) {
						...
						$("#liti_secondary_responsible").html(data);
						$("#liti_assigned_to").html(data);
						$("#liti_third_responsible_person").html(data);
						$("#liti_fourth_responsible_person").html(data);
					},
					error : function(xhr, ajaxOptions, thrownError) {
						$('#errors').html("There is error:" + thrownError);
					}
		...
		$('#liti_secondary_responsible').html('<option value="0">--Select--</option>');
		$('#liti_assigned_to').html('<option value="0">--Select--</option>');
		$('#liti_third_responsible_person').html('<option value="0">--Select--</option>');
		$('#liti_fourth_responsible_person').html('<option value="0">--Select--</option>');
	}
});
...
function getAllLitigationDocuments(data){
	...
	$.ajax({
		...
		success : function(documentsList) {
			...
			var htmlOutputLitigationRelated6 = '';
			...
			if(documentsList != ""){
				...
				for(var i=0 ; i < documentJson.length ; i++){
					...
					/*else if(innerObj["ldoc_document_type"] == "LitigationDoc"){
						htmlOutputLitigationRelated1 += "<tr id='litiDocRow_"+innerObj["ldoc_id"]+"'><td>"+ j +"</td>";
						htmlOutputLitigationRelated1 += "<td><a href='./downloadLitigationDocument?ldoc_id="+innerObj["ldoc_id"]+"'>"+ innerObj["ldoc_original_file_name"] +"</a></td>";
						htmlOutputLitigationRelated1 += "<td class='delete'><button type='button' value='"+ innerObj["ldoc_id"] +"' name='delete_litigation_document' class='btn btn-danger'><i class='fa fa-trash'></i>&nbsp;Delete</button></td></tr>";
						j++;
					...
					else if(innerObj["ldoc_document_type"] == "LitigationCaseDocuments"){
						htmlOutputLitigationRelated6 += "<tr id='litiDocRow_"+innerObj["ldoc_id"]+"'><td>"+ q +"</td>";
						htmlOutputLitigationRelated6 += "<td><a href='./downloadLitigationDocument?ldoc_id="+innerObj["ldoc_id"]+"'>"+ innerObj["ldoc_original_file_name"] +"</a></td>";
						htmlOutputLitigationRelated6 += "<td class='delete'><button type='button' value='"+ innerObj["ldoc_id"] +"' name='delete_litigation_document' class='btn btn-danger'><i class='fa fa-trash'></i>&nbsp;Delete</button></td></tr>";
						p++;
					}
			...
			$("#liti_docs_tbody6").html(htmlOutputLitigationRelated6);
...
/*$("#liti_amount_involved").on("change", getConversionRate);
$("#liti_interest").on("change", getConversionRate);*/

function getConversionRate(){
	
	var amount_involved = $('#liti_amount_involved').val();
	var interest = $("#liti_interest").val();
	var liti_total_amount = $('#liti_total_amount').val();
				...
				success : function(result) {
					 if(result.length != 0){ 
							 var rate = result;
								  ...
								  $("#liti_interest").val(rate);
								  
								  $("#liti_total_amount").val(((rate /100) * amount_involved) + parseInt(amount_involved));
								  if (rate == 0) {
							         	 
							      		$( "#liti_interest" ).attr( "data-placement", "bottom" );
							      		$( "#liti_interest" ).attr( "data-content", "Conversion rate is not avaliable." );
							      		$('#liti_interest').popover('show');
							    	
							             return false;
							        }else{
							          	$('#liti_interest').popover('destroy');
							        }
					 }
				},error : function(xhr, ajaxOptions, thrownError) {
					$('#errors').html("There is error:" + thrownError);
...
$("#liti_interest").change(function(){
	var liti_amount_involved = $('#liti_amount_involved').val();
	var liti_interest = $("#liti_interest").val();
	if(liti_interest !=''){
	var liti_interest = $('#liti_interest').val();
	$("#liti_total_amount").val(((liti_interest/100)*liti_amount_involved)+parseInt(liti_amount_involved));
	}else{
		$("#liti_interest").val(0.0);
	}
});
...
function validateForm() { 
  ...
  //  var liti_advocate_id = $('#liti_advocate_id').val();
    ...
    var liti_receipt_notice_date = $('#liti_receipt_notice_date').val();
  ...
  //  var liti_relevant_law = $('#liti_relevant_law').val();
    ...
    var liti_total_amount = $('#liti_total_amount').val();
    var liti_interest = $('#liti_interest').val();
    var liti_prayer_details = $('#liti_prayer_details').val();
    var liti_acm_note_date = $('#liti_acm_note_date').val();
    ...
    if (liti_type_id == 0 ) {
  		...
  		$( "#liti_type_id" ).attr( "data-content", "Please select mandatory fields." );
    ...
    if (liti_assigned_to == 0 || liti_assigned_to =='' ) {
   	 
  		$( "#liti_assigned_to" ).attr( "data-placement", "top" );
  		$( "#liti_assigned_to" ).attr( "data-content", "Please enter Primary Responsible Person" );
  		$('#liti_assigned_to').popover('show');
	
         return false;
     }
 	 else
      {
      	$('#liti_assigned_to').popover('destroy');
      }
    ...
    if (liti_party_against == null || liti_party_against ==''  ) {
  		...
  		$( "#liti_party_against" ).attr( "data-content", "Please enter Against Party." );
    ...
    if (liti_case_filing_date == 0 ) {
  		...
  		$( "#liti_case_filing_date" ).attr( "data-content", "Please select Case Filing Date." );
    ...
    if (liti_receipt_notice_date == 0 ) {
    	 
  		$( "#liti_receipt_notice_date" ).attr( "data-placement", "top" );
  		$( "#liti_receipt_notice_date" ).attr( "data-content", "Please select Receipt of Notice date." );
  		$('#liti_receipt_notice_date').popover('show');
	
         return false;
     }
 	 else
      {
      	$('#liti_receipt_notice_date').popover('destroy');
      }
  ...
  /*  if (liti_criticality == 0 ) {
  		...
  		$( "#liti_criticality" ).attr( "data-content", "Please select Risk Analysis." );
    ...
    */
    if (liti_court == 0 ) {
  		...
  		$( "#liti_court" ).attr( "data-content", "Please select Court	.");
  ...
  /*  if (liti_interest == null || liti_interest == "") {
    	 
  		$( "#liti_interest" ).attr( "data-placement", "top" );
  		$( "#liti_interest" ).attr( "data-content", "Please enter the Interest." );
  		$('#liti_interest').popover('show');
				...
				success : function(result) {
					 if(result.length != 0){ 
							 ...
							 //alert("£1 =" + rate)
								  $("#liti_interest").val(rate);
								  //var conversion_rate = 
								  $("#liti_total_amount").val((rate /100)+ amount_involved);
								  if (rate == 0) {
...
$("#liti_interest").change(function(){
	...
	if(liti_interest !=''){
	var liti_interest = $('#liti_interest').val();
	$("#liti_interest").val((liti_interest/100)+liti_amount_involved);
	}else{
		$("#liti_interest").val(0.0);
	}
});
...
$("#liti_amount_involved").keyup(function(){
	var liti_interest = $("#liti_interest").val();
	var liti_amount_involved = $('#liti_amount_involved').val();
	$("#liti_total_amount").val((liti_interest/100)+liti_amount_involved);
});

*/

var litigationFileCount = 1;
function addLitigationFileInput(){
	$("#fileContiner_Legal").append('<div class="col-md-12" id="litigationFile'+litigationFileCount+'" >'
			+'<div class="form-group">'
	        +'<label class="col-sm-4 control-label">Litigation:</label>'
	        +'<div class="col-sm-4">'
	        +'<input type="file" name="litigation_doc" class="file-loading" />'
	        +'</div>'
	        +'<div class="col-sm-3" style="text-align: right; cursor: pointer;">'
	        +'<i class="glyphicon glyphicon-remove-circle"	onclick="deleteLitigationRow('+litigationFileCount+');" style="color: red;" title="Delete"></i>'
	        +'</div>'
	        +'</div>'
	        +'</div>'
			);
...
function deleteLitigationRow(filecount) {
